...
erf <- function(x) 2 * pnorm(x * sqrt(2)) - 1  # ERROR FUNTION
...
gauss01 <- function(x,mu,sigma,prmax=1.0,prmin=0.0)
{
	
	y <-  (prmax-prmin)*exp(-((x - mu)^2/(2* sigma^2))) + prmin
	y
}

gauss01.lms <- function(p,x,y,weight=NULL)
{
...
cgauss_lapse <- function(x,p,escala=0.0,k=NULL) {
  
  p[3]+(1-p[3]-p[4])* ((1-escala)*pnorm(x,p[1],p[2])+escala)
}

cgauss_lapse.lms   <- function(p,x,y,weight=1,escala=0.0,k=NULL) { 
  #  w <- 1
  
  w <- weight/sum(weight)
  yhat<- p[3]+(1-p[3]-p[4])* (1-escala)*pnorm(x,p[1],p[2]) + escala 
...
#plotting function Cgauss with intervals
cgauss_laps <- function(x,p,k=NULL) {   #(x,u,var,gamma,lambda)
  
  z <- (x - p[1])/sqrt(p[2])
  old_prob <- 0.5 + erf(z/sqrt(2))/2 #normal cumulatuve
  
prob <- p[3]+(1-p[3]-p[4])*old_prob
  
}
  

#x <- seq(0,50, by=0.01)
#y <- cgauss_laps(x,c(30, 5,0.3, 0.1))

cgauss_laps.lms   <- function(p,x,y,weight=60) { 
  #  w <- 1
  
  w <- weight/sum(weight) 
  z <- (x - p[1])/sqrt(p[2])
  old_prob <- 0.5 + erf(z/sqrt(2))/2 #normal cumulatuve  
  yhat <- p[3]+(1-p[3]-p[4])*old_prob
