...
mixed_slope_triangle <- function(x,central,p){
  leftsidey <- p[1]*x + p[3] 
  nelements_left <- len(x[central>=x])
  nelements_right <- len(x[central<=x])
  elementes_right <- -rev(x[central<=x] - central)
  elementes_right <- elementes_right + central
  rightsidey <-  p[1]*elementes_right +  p[3] 
  rightsidey <-  rev(rightsidey)
  ylr <- c(leftsidey[1:nelements_left],rightsidey[-1])
  len(ylr) 
  generaltrend =  ylr + p[2]*x 
  generaltrend
...
mixed_slope_triangle.lms <- function(p,x,y,central,weight=1,k=NULL)
{
  leftsidey <- p[1]*x +  p[3] 
  nelements_left <- len(x[central>=x])
  nelements_right <- len(x[central<=x])
...
mixed_slope_triangle.nls <- function(p,x,y,central,weight=1,k=NULL)
{
  leftsidey <- p[1]*x +  p[3] 
  nelements_left <- len(x[central>=x])
  nelements_right <- len(x[central<=x])
...
fit <-  nls(~ mixed_slope_triangle.nls(p),x)
...
pieron.optim.fit <- function(Pieron, vars=NULL,ffunc=pieron.lms,ipars=c(1,1,1),dfunc=pieron,k=NULL,... )
{
  ...
  fit <- optim(fit,ffunc,x=Pieron[,f1[1]],y=Pieron[,f1[2]],weight=1/Pieron[,f1[3]],k=k,hessian=TRUE,...)          #pasamos los primeros parametros estimados otra vez a optim, utilizamos 1/weight para ponderar en funci?n del error de cada punto
...
lmodes.fit <- function(Lineal, vars=NULL)
{
  ...
  fits <- summary(lm(Lineal[,f1[2]]~Lineal[,f1[1]]))$coefficients[2] 
  fit <- summary(lm(Lineal[,f1[2]]~Lineal[,f1[1]]))$coefficients[4]# puntuaciones colocadas en escala logaritmica
